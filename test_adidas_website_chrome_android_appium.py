...
def test_user_reg(driver):
    ...
    driver.execute_script("arguments[0].click();", WebDriverWait(driver, 20).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "span > .gl-cta > span"))))
    time.sleep(4)
    ...
    time.sleep(2)
    driver.find_element(By.ID, "login-email").send_keys("d_test_pytest@gmail.com")
    time.sleep(2)
...
def test_buy_product_feature_with_guest_account(driver):
    ...
    elem_img = driver.find_element(By.CSS_SELECTOR, ".col-6:nth-child(10) .tile-image")
    time.sleep(1)
    actions.move_to_element(elem_img).perform()
    time.sleep(2)
    elem_img.click()
    time.sleep(3)
    driver.find_element(By.CSS_SELECTOR, ".swatch-rect-carbon-\\/-white").click()
...
def test_add_to_wishlist_after_login(driver):
    ...
    driver.find_element(By.LINK_TEXT, "T-Shirts & Tops").click()
    time.sleep(4)
    heart_button = driver.find_element(By.CSS_SELECTOR,
                                       "#product-search-results > div.row.no-gutters > div.col-sm-12.col-md-9.custom-padding > div.container.p-0 > div > div.row.product-wrapsection > div:nth-child(2) > div > div > div.image-container > a.wishlistTile > div > img.heart-empty.js-heart")
    driver.execute_script("arguments[0].click();", heart_button)
    time.sleep(3)
    ...
    login_message_wishlist = driver.find_element(By.XPATH,
                                                 "//*[@id=\"maincontent\"]/div/div/div/div[1]/div[2]/div/div[3]/div/p")
    actions.move_to_element(login_message_wishlist).perform()
    time.sleep(1)
    login_message_wishlist = driver.find_element(By.XPATH,
...
def test_Total_Price_is_correct(driver):
    ...
    two_items_price = float(two_items_price[2:8])
    total_price = driver.find_element(By.XPATH,
                                      "//*[@id=\"maincontent\"]/div/div[4]/div[3]/div[2]/div[6]/div[5]/div[2]/p").text
    total_price = float(total_price[2:8])
    delivery_cost = driver.find_element(By.CSS_SELECTOR,
                                        "#maincontent > div > div.container.cart.cart-page > div.row > div.col-lg-5.col-xl-4.totals > div.order-totals-main-section > div:nth-child(3) > div:nth-child(2) > p").text
    if delivery_cost != 'FREE':
        delivery_cost = float(delivery_cost[2:8])
        two_items_price += delivery_cost
    time.sleep(2)
...
def test_Total_Price_is_correct_delivery_free(driver):
    driver.get("http://www.adidas.co.il/en")
    driver.find_element(By.CSS_SELECTOR, ".affirm").click()
    time.sleep(2)
    driver.find_element(By.CSS_SELECTOR, ".navbar-toggler:nth-child(1)").click()
